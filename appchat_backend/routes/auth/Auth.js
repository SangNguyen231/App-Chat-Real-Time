...
routerAuth.post("/confirm_phone", async (req, res) => {
  try {
    const { phone } = req.body;
    const oldUser = await User.findOne({ phone });

    if (oldUser) {
      return res.status(200).send("Phone is available");
    } else return res.status(400).send("Phone is not available in system.");
  } catch (error) {
    console.log(err);
  }
});
