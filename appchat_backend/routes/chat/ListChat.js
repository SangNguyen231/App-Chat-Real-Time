...
routerListChat.put("/addmember", verifyToken, async (req, res) => {
  try {
    let { room_id, list_member } = req.body;

    const temp = await Room.findById(room_id);
    const temp1 = Object.keys(temp.list_member).length;
    const temp2 = temp1 + list_member.length;

    let [newList_member] = ([] = await Promise.all(
      list_member.map(async (course, index) => {
        let data_user = await User.findOne({ phone: course.phone });
        list_member[index].avatar = data_user.avatar;
        list_member[index].nickname = `${data_user.first_name.trim()} ${data_user.last_name.trim()}`;
        return list_member;
      })
    ));
...
//Lay toan bo hinh anh da gui trong room chat
routerListChat.post("/allImages", verifyToken, async (req, res) => {
  try {
    const { _id } = req.body;
    ...
    let list_Image = [];
    const list_message = room.list_message;
    list_message.forEach((item) => {
      if (
        item.type_message.type === "image" &&
        !item.is_removes.some((course) => course.phone === req.user.phone)
      ) {
        list_Image.push(item);
      }
    });
    return res.status(201).send(list_Image);
  } catch (error) {
    return res.status(400).send(error);
  }
});

//Lay toan bo file da gui trong room
routerListChat.post("/allFiles", verifyToken, async (req, res) => {
  try {
    const { _id } = req.body;
    const room = await Room.findOne({
      _id: _id,
    ...
    list_message.forEach((item) => {
      if (
        item.type_message.type === "files" &&
        !item.is_removes.some((course) => course.phone === req.user.phone)
      ) {
        list_Files.push(item);
      }
    });
    return res.status(201).send(list_Files);
  } catch (error) {
    return res.status(400).send(error);
  }
});
...
//get room chat team
routerListChat.get("/TeamRoom", verifyToken, async (req, res) => {
  
  try {
    //Lay room co user
    const roomChat = await Room.find({ "list_member.phone": req.user.phone });
    //Name room khac isFriend
    let list_RoomTeam = [];
    roomChat.forEach((item) => {
      if(item.name_room !== "isFriend") {
        list_RoomTeam.push(item);
      }
    })
    console.log(list_RoomTeam);
    return res.status(201).send(list_RoomTeam);
  } catch (error) {
    res.send(error);
  }
});
...
routerListChat.post("/rename_Friend", verifyToken, async (req, res) => {
  try {
    const { _id, nick_name, phone_friend } = req.body;

    const list_room = await Room.findOne({
      _id: _id,
    });

    let New_listmember = list_room.list_member;
    New_listmember.forEach((course, index) => {
      if (course.phone === phone_friend) {
        New_listmember[index].nickname = nick_name;
      }
    });

    const new_Room = await Room.findOneAndUpdate(
      { _id: _id },
      {
        $set: {
          list_member: New_listmember,
        },
      }
    );

    return res.status(201).send(new_Room);
  } catch (error) {
    return res.status(400).send(error);
  }
});
