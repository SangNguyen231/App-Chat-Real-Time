...
io.on("connection", (socket) => {
  ...
  socket.on("add_friend", (data_receive, data_send) => {
    io.sockets
      .in(data_receive.phone)
      .emit("receive_addFriend", data_receive, data_send);
    io.sockets
      .in(data_send.phone)
      .emit("receive_Send", data_receive, data_send);
  });

  socket.on("add_member", (data) => {
    let list_Newmember = [];
    data.data.list_member.forEach((course, index) => {
      data.list_memberNew.forEach((item) => {
        course.phone === item.phone &&
          list_Newmember.push(data.data.list_member[index]);
      });
    });
    io.sockets
      .in(data.room_id)
      .emit("receive_addMember", data.room_id, list_Newmember);
    data.list_memberNew.forEach((course) => {
      io.sockets.in(course.phone).emit("receive_addMember", data.data);
    });
  });

  socket.on("edit_profile", (phone) => {
    io.sockets.in(phone).emit("receive_editProfile", phone);
  });
